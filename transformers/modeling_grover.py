...
def gelu_rowan(x):
    assert(False)
    # issue: how do we calculate erf? Just assume rowan's and gpt2's is equivalent
...
class Attention(nn.Module):
    def __init__(self, nx, n_ctx, config, scale=False):
        ...
        self.attn_proj_query = nn.Linear(nx, n_state)
        self.attn_proj_key = nn.Linear(nx, n_state)
        self.attn_proj_value = nn.Linear(nx, n_state)
        
#        self.attn_dropout = nn.Dropout(config.attn_pdrop)
        self.hidden_dropout = nn.Dropout(config.resid_pdrop)
    ...
    def forward(self, x, layer_past=None, attention_mask=None, head_mask=None):
        ...
        # !!! PT_2_0
        # x = self.c_attn(x) Not convolution, dense instead. Figure out what this size would be!!!
        # PT_2_0_0
        query = self.attn_proj_query(x).view(-1, )
        key = 
        value = 
        
        
        # Note: split splits it along the given dimension
...
# todo PT_3 (mlp block)
class rediual_MLP_layer(nn.Module):
...
class GroverModel(GPT2PreTrainedModel):
    r"""
    ...
    def __init__(self, config):
        ...
        self.ln_f = nn.LayerNorm(config.n_embd, eps=1e-5) # CHANGE from gpt2, in grover this is always 1e-5
...
class GroverLMHeadModel(GPT2PreTrainedModel):
    r"""
