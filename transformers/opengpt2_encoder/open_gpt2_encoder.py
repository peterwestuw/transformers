...
class Encoder:
    ...
    '''
    NOTE: the token + 1 to take into account the shift that allows for padding in tensorflow
    '''
    def encode(self, text):
        bpe_tokens = []
        for token in re.findall(self.pat, text):
            token = ''.join(self.byte_encoder[b] for b in token.encode('utf-8'))
            bpe_tokens.extend(self.encoder[bpe_token] + 1 for bpe_token in self.bpe(token).split(' '))
        return bpe_tokens
    '''
    NOTE: the token - 1 to take into account the shift that allows for padding in tensorflow
    '''
    def decode(self, tokens):
        text = ''.join([self.decoder[token - 1] for token in tokens])
