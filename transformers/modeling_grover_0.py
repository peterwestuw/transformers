...
def load_pretrained_grover(config, grover_checkpoint_path):
    ...
    if config is 'base':
        config = GroverConfig.from_json_file(file_path+'/grover_configs/base.json')
    elif config is 'medium':
        config = GroverConfig.from_json_file(file_path+'/grover_configs/medium.json')
...
class Attention(nn.Module):
    ...
    def _attn(self, q, k, v, attention_mask=None, head_mask=None):
        ...
        # !!! PT_2_2 could be wrong! (should be sqrt size_per_head??) Looks rights though... (see "split_heads" for value)
        if self.scale:
...
#            w = w * head_mask
    ...
    def forward(self, x, layer_past=None, attention_mask=None, head_mask=None):
        ...
        # PT_2_0 !!! could be wrong! (but very similar)
        query, key, value = self.c_attn_q(x), self.c_attn_k(x), self.c_attn_v(x)
        ...
        # !!! PT_2_6 This could be wrong (might be a simpler dense layer in fakenews)
        a = self.c_proj(a)
...
# this will comgine the elements of the layer loop
# in og grover code 
...
class GroverPreTrainedModel(PreTrainedModel):
    ...
    """
    config_class = GroverConfig
    pretrained_model_archive_map = GPT2_PRETRAINED_MODEL_ARCHIVE_MAP
    load_tf_weights = load_tf_weights_in_gpt2
    base_model_prefix = "transformer"

    def __init__(self, *inputs, **kwargs):
        super(GroverPreTrainedModel, self).__init__(*inputs, **kwargs)
...
class GroverModel(GroverPreTrainedModel):
    r"""
...
#        hidden_states = self.drop(hidden_states)
        ...
        # this is part of residual_MLP in PT newslm
...
class GroverLMHeadModel(GroverPreTrainedModel):
    r"""
...
'''Removing this block for now, because we are only planning to use Grover as
an LM for the time being'''
